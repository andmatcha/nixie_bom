...
SN75468 の出力耐圧 100 V に対して余裕を持たせるため、消灯中のカソードラインは約 85 V にプリバイアスする。
...
170 V 入力時の無負荷 VPRE は約 85.0 V、180 V 入力時は約 90.0 V となる。1 % 抵抗の最悪側でも 180 V 入力時の VPRE は約 90.9 V に収まるため、SN75468 の 100 V 定格に対する静的な余裕を確保しやすい。点灯中の選択カソードラインは 1 MOhm 経由で VPRE を負荷するため、実動作時の VPRE と SN75468 出力端子電圧は必ず実測する。
...
- RAHOFFxA/RAHOFFxB: 499 kOhm x 2 直列、PNP ベース-エミッタ間プルアップ
...
RAHOFF も PNP オン時に 170 V 系の電位差を受けるため、1 MOhm 単体ではなく 499 kOhm x 2 直列とする。170 V 時は各抵抗の印加電圧が約 85 V になり、ERJ-6EN の制限素子電圧 150 V に対して余裕を持てる。
...
| RAHOFF1A-RAHOFF6B | 12 | アノード PNP オフ保持 | Panasonic | [ERJ-6ENF4993V](https://www.digikey.jp/ja/products/detail/panasonic-industry/ERJ-6ENF4993V/112126) | 499 kOhm, 1 %, 0805、2 本直列/桁 | アクティブ、在庫あり |
| RKP1-RKP10 | 10 | カソードプリバイアス | Panasonic | [ERJ-6ENF1004V](https://www.digikey.jp/ja/products/detail/panasonic-industry/ERJ-6ENF1004V/112223) | 1.00 MOhm, 1 %, 0805 | アクティブ、在庫あり |
...
| RPREH1, RPREL1 | 2 | VPRE 分圧 | Panasonic | [ERJ-6ENF1803V](https://www.digikey.jp/ja/products/detail/panasonic-industry/ERJ-6ENF1803V/1746469) | 180 kOhm, 1 %, 0805 | アクティブ、在庫あり |
...
| RAHOFF の 1.00 MOhm 単体構成 | PNP オン時に 170 V 系の電位差を受け、0805 抵抗単体の制限素子電圧に対する余裕が不足するため、499 kOhm x 2 直列を採用する |
| RPREH1 = 150 kOhm | 180 V 側で VPRE が 100 V に近づくため、SN75468 出力耐圧の余裕を増やす目的で 180 kOhm へ変更する |
...
- RAHDRV1A-RAHDRV6B と RAHOFF1A-RAHOFF6B は 170 V 系に関わるため、直列 2 本の配置間隔と沿面距離を確保する。
- RA1-RA6 と RINS1-RINS4 は発熱と輝度を実測し、必要に応じて値を変更する。
- VPRE は 170 V 出力の変動に追従するため、170 V が 180 V 側に上がった条件でも SN75468 出力定格に入ることを確認する。初期定数では 180 V 時の無負荷 VPRE が約 90 V になる。
